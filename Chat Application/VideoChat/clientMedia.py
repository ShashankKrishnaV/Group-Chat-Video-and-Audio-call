...
def MyThread():
    while True:
        frame1 = wvs.read()
        cv2_im = cv2.cvtColor(frame1, cv2.COLOR_BGR2RGB)
        frame1 = cv2.resize(frame1, (640, 480))
        frame1 = np.array(frame1, dtype = np.uint8).reshape(480, 640, 3)
        cv2.imshow("Myself", frame1)
        if cv2.waitKey(1) == 27:
            cv2.destroyAllWindows()

def SendFrame():
    MyselfThread = Thread(target=MyThread).start()
    while True:
        try:
            frame = wvs.read()
...
def func(IP):
    global clientVideoSocket
    global wvs
    global clientAudioSocket
    global audio
    global stream
    global SendFrameThread, SendAudioThread, ReceiveFrameThread, ReceiveAudioThread
    HOST = IP
